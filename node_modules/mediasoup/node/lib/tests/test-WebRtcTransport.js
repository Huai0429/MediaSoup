...
test('router.createWebRtcTransport() succeeds', async () => {
    ...
    const anotherTransport = await router.createWebRtcTransport({ listenIps: ['127.0.0.1'] });
...
test('WebRtcTransport events succeed', async () => {
    ...
    channel.emit(transport.id, 'dtlsstatechange', { dtlsState: 'connected', dtlsRemoteCert: 'ABCD' });
